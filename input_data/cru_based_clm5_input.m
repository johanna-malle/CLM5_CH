function regrid_crujura_lp
%{
Script to regrid precip data from global crujura V2.2. dataset to the 1km grid
over the extent of Switzerland... done with linear interpolation for now.
...
% new domain
dom_data='/media/malle/LaCie/CLM5_input/domain.lnd.CH_1km_navy.210407_new.nc';

%new grid (vertices)
LONGXY_new=ncread(dom_data,'xc');
LATIXY_new=ncread(dom_data,'yc');
LONGXY_vert_new=ncread(dom_data,'xv');
LATIXY_vert_new=ncread(dom_data,'yv');
...
% ... and output data
bafu_DEM='/media/malle/LaCie/OSHD_info/BAFU_DEM_2020_1000.txt';
[DEM_CH,~]= readgeoraster(bafu_DEM);
DEM_CH(DEM_CH(:,:)<0)=0; %grid cells outside of domain => just leave it at the sealevel temperature
%%
% list={'lwr_pa','precip','solar','tair_rh','wind_zbot'};
list={'tair_rh'};
...
for oix=1:size(list,2)
    ...
    bf_out=fullfile('/media/malle/LaCie/CLM5_input/cru_jra_CLM5_CH/',var);
    if strcmp(var,'tair_rh')==1
        bf_out_lapse=fullfile('/media/malle/LaCie/CLM5_input/cru_jra_CLM5_CH/',[var '_lapse']);
    end
    ...
    for six=1:size(names_data,1)
        name_six=names_data(six);
        file_world=fullfile(bf_in,cell2mat(name_six));
        file_ch=fullfile(bf_out,cell2mat(name_six));
        if strcmp(var,'tair_rh')==1
            file_ch_lapse=fullfile(bf_out_lapse,cell2mat(name_six));
            if exist(file_ch_lapse,'file')==2
                delete(file_ch_lapse)
            end
        else
            file_ch_lapse=[];
        end
        if exist(file_ch,'file')==2
            delete(file_ch)
        end
        ...
        file_comp={file_ch,file_ch_lapse};

        for fix=1:2
            file_ch_all=file_comp{fix};
            if ~isempty(file_ch_all)
        ...
        if strcmp(var,'lwr_pa')==1
            ...
            nccreate(file_ch,'PSRF','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
            ncwriteatt(file_ch,'PSRF', 'long_name',ncreadatt(file_world,'PSRF','long_name'));
            ncwriteatt(file_ch,'PSRF', 'units', ncreadatt(file_world,'PSRF','units'));
            ncwriteatt(file_ch,'PSRF', 'mode',ncreadatt(file_world,'PSRF','mode'));
            ncwriteatt(file_ch,'PSRF', '_FillValue',ncreadatt(file_world,'PSRF','_FillValue'));
            ncwriteatt(file_ch,'PSRF', 'missing_value',ncreadatt(file_world,'PSRF','missing_value'));
            ncwrite(file_ch,'PSRF',PSRF_regrid);

            nccreate(file_ch,'FLDS','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
            ncwriteatt(file_ch,'FLDS', 'long_name',ncreadatt(file_world,'FLDS','long_name'));
            ncwriteatt(file_ch,'FLDS', 'units', ncreadatt(file_world,'FLDS','units'));
            ncwriteatt(file_ch,'FLDS', 'mode',ncreadatt(file_world,'FLDS','mode'));
            ncwriteatt(file_ch,'FLDS', '_FillValue',ncreadatt(file_world,'FLDS','_FillValue'));
            ncwriteatt(file_ch,'FLDS', 'missing_value',ncreadatt(file_world,'FLDS','missing_value'));
            ncwrite(file_ch,'FLDS',FLDS_regrid);
        ...
        elseif strcmp(var,'precip')==1
            ...
            nccreate(file_ch,'PRECTmms','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
            ncwriteatt(file_ch, 'PRECTmms', 'long_name', 'PRECTmms total precipitation');
            ncwriteatt(file_ch, 'PRECTmms', 'units', 'mm H2O / sec');
            ncwriteatt(file_ch, 'PRECTmms', 'mode', 'time-dependent');
            ncwriteatt(file_ch, 'PRECTmms', '_FillValue',9.999999616903162e+35);
            ncwriteatt(file_ch, 'PRECTmms', 'missing_value',9.999999616903162e+35);
            ncwrite(file_ch,'PRECTmms',PRECIP_regrid);
        ...
        elseif strcmp(var,'solar')==1
            ...
            nccreate(file_ch,'FSDS','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
            ncwriteatt(file_ch, 'FSDS', 'long_name', 'total incident solar radiation');
            ncwriteatt(file_ch, 'FSDS', 'units', 'W/m**2');
            ncwriteatt(file_ch, 'FSDS', 'mode', 'time-dependent');
            ncwriteatt(file_ch, 'FSDS', '_FillValue',9.999999616903162e+35);
            ncwriteatt(file_ch, 'FSDS', 'missing_value',9.999999616903162e+35);
            ncwrite(file_ch,'FSDS',FSDS_regrid);

        elseif strcmp(var,'tair_rh')==1
            ...
            for miix=1:sz_t
                ...
                t_sealevel=flip(temp_interp)+(6.5/1000)*flip(TOPO_cru7); %global topo to bring temperature down to sealevel
                T_lapse_rate=t_sealevel-(6.5/1000)*DEM_CH;         %use DEM of Switzerland to "relapse"
                TBOT_regrid_lapse(:,:,miix)=flip(T_lapse_rate)';

                QBOT_inter=interp2(X_llc,Y_llc,(QBOT(:,:,miix))',X_new_llc,Y_new_llc);
                QBOT_regrid(:,:,miix)=QBOT_inter';
            end

%             nccreate(file_ch,'TBOT','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
%             ncwriteatt(file_ch, 'TBOT', 'long_name', 'temperature at the lowest atm level');
%             ncwriteatt(file_ch, 'TBOT', 'units', 'K');
%             ncwriteatt(file_ch, 'TBOT', 'mode', 'time-dependent');
%             ncwriteatt(file_ch, 'TBOT', '_FillValue',9.999999616903162e+35);
%             ncwriteatt(file_ch, 'TBOT', 'missing_value',9.999999616903162e+35);
%             ncwrite(file_ch,'TBOT',TBOT_regrid);
%
%             nccreate(file_ch,'QBOT','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
%             ncwriteatt(file_ch, 'QBOT', 'long_name', 'specific humidity at the lowest atm level');
%             ncwriteatt(file_ch, 'QBOT', 'units', 'kg/kg');
%             ncwriteatt(file_ch, 'QBOT', 'mode', 'time-dependent');
%             ncwriteatt(file_ch, 'QBOT', '_FillValue',9.999999616903162e+35);
%             ncwriteatt(file_ch, 'QBOT', 'missing_value',9.999999616903162e+35);
%             ncwrite(file_ch,'QBOT',QBOT_regrid);

            nccreate(file_ch_lapse,'TBOT','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
            ncwriteatt(file_ch_lapse, 'TBOT', 'long_name', 'temperature at the lowest atm level');
            ncwriteatt(file_ch_lapse, 'TBOT', 'units', 'K');
            ncwriteatt(file_ch_lapse, 'TBOT', 'mode', 'time-dependent');
            ncwriteatt(file_ch_lapse, 'TBOT', '_FillValue',9.999999616903162e+35);
            ncwriteatt(file_ch_lapse, 'TBOT', 'missing_value',9.999999616903162e+35);
            ncwrite(file_ch_lapse,'TBOT',TBOT_regrid_lapse);

            nccreate(file_ch_lapse,'QBOT','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
            ncwriteatt(file_ch_lapse, 'QBOT', 'long_name', 'specific humidity at the lowest atm level');
            ncwriteatt(file_ch_lapse, 'QBOT', 'units', 'kg/kg');
            ncwriteatt(file_ch_lapse, 'QBOT', 'mode', 'time-dependent');
            ncwriteatt(file_ch_lapse, 'QBOT', '_FillValue',9.999999616903162e+35);
            ncwriteatt(file_ch_lapse, 'QBOT', 'missing_value',9.999999616903162e+35);
            ncwrite(file_ch_lapse,'QBOT',QBOT_regrid);
        ...
        elseif strcmp(var,'wind_zbot')==1
            ...
            nccreate(file_ch,'WIND','Dimensions',{'lon',size(LONGXY_new,1),'lat',size(LONGXY_new,2),'time',length_time},'Format','classic','Datatype','double');
            ncwriteatt(file_ch, 'WIND', 'long_name', 'wind at the lowest atm level');
            ncwriteatt(file_ch, 'WIND', 'units', 'm/s');
            ncwriteatt(file_ch, 'WIND', 'mode', 'time-dependent');
            ncwriteatt(file_ch, 'WIND', '_FillValue',9.999999616903162e+35);
            ncwriteatt(file_ch, 'WIND', 'missing_value',9.999999616903162e+35);
            ncwrite(file_ch,'WIND',Wind_regrid);
        end
    end
end
end
