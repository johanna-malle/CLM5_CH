function make_wiggle_plot_new_data_rev
close all
clearvars
%{
...
for tix=1:sz_locs %loop through all locations of interest
    ...
    fh=figure('Position',[963 339 521 629]);hold on;grid on
    ...
    pos_2=ah2.Position;
    ah2.Position=[pos_2(1) pos_2(2)-0.07 pos_2(3:4)];
    ...
    set(gcf,'Color','white')
    le1.Position=[0.0399 0.42 0.9360 0.0964];
    le1.NumColumns=3;
    set(findall(gcf,'-property','FontSize'),'FontSize',14.5)
    le1.FontSize=10.5;
    %%
...
save(fullfile(bf_out,'mat_for_wiggle.mat'),'timing_check','timing_check_crujra',...
    'elev_inter','mat_all_crujra_newSurf','mat_all_crujra_origSurf',...
...
for jiix=2:4%size(mat_all_jim,3) %2014 not actually proper data
    ...
    an4=annotation('textbox',[0.6692 0.0938 0.2610 0.0845],'String',{['min \Delta = ' num2str(round(min_all_plot,2)) 'm'],...
    ...
    set(gcf,'Color','white')
    set(findall(gcf,'-property','FontSize'),'FontSize',10.3)

    ti(1)=title(ah1,'Observed - FSM2');
    ti(2)=title(ah2,'Observed - CLM5  {\itClim_{OSHD^{pt}} LU_{HR}}','interpreter','tex');
    ti(3)=title(ah3,'Observed - CLM5 {\itClim_{CRU*^{pt}} LU_{HR}}','interpreter','tex');
    ti(4)=title(ah4,'Observed - CLM5 {\itClim_{CRU^{pt}} LU_{HR}}','interpreter','tex');
    set(ti,'FontSize',12,'Color',rgb('red'),'FontWeight','Normal')

    ah1.Position=[0.187 0.7873 0.7561 0.15];
    ah2.Position=[0.187 0.5770 0.7561 0.15];
    ah3.Position=[0.187 0.368282 0.7561 0.15];
    ah4.Position=[0.187 0.165164 0.7561 0.15];

    fh.Position = [486 105 517 575];
    an5.Position = [0.0269 0.0424 0.4969 0.0400];
    an4.Position = [0.6794 0.01036 0.286199 0.0726];
    an4.FontSize = 12;
    an5.FontSize = 12;
    %%
    ...
    set(gcf,'Color','white')
    set(findall(gcf,'-property','FontSize'),'FontSize',9.3)

    ti(1)=title(ah1,'Observed - FSM2');
    ti(2)=title(ah2,'Observed - CLM5  {\itClim_{OSHD^{pt}} LU_{HR}}','interpreter','tex');
    ti(3)=title(ah3,'Observed - CLM5 {\itClim_{CRU*^{pt}} LU_{HR}}','interpreter','tex');
    ti(4)=title(ah4,'Observed - CLM5 {\itClim_{CRU^{pt}} LU_{HR}}','interpreter','tex');
    set(ti,'FontSize',12,'Color',rgb('blue'),'FontWeight','Normal');

    ah1.Position=[0.05 0.7873 0.88 0.15];
    ah2.Position=[0.05 0.5770 0.88 0.15];
    ah3.Position=[0.05 0.368282 0.88 0.15];
    ah4.Position=[0.05 0.165164 0.88 0.15];
    ...
    set(gcf,'Color','white')
    set(findall(gcf,'-property','FontSize'),'FontSize',7.5)
    set(an4,'FontSize',8)
    ...
    ti(3)=title(ah3,'Observed - CLM5 {\itClim_{CRU*^{pt}} LU_{HR}}','interpreter','tex');
    ti(4)=title(ah4,'Observed - CLM5 {\itClim_{CRU^{pt}} LU_{HR}}','interpreter','tex');
    set(ti,'FontSize',12,'Color',rgb('green'),'FontWeight','Normal');
    ah1.Position=[0.01 0.7873 0.95 0.15];
    ah2.Position=[0.01 0.5770 0.95 0.15];
    ah3.Position=[0.01 0.368282 0.95 0.15];
    ah4.Position=[0.01 0.165164 0.95 0.15];
